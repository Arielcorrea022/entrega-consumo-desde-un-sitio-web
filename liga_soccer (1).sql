...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 05-09-2025 a las 23:39:38
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_estadio` (IN `p_nombre` VARCHAR(100), IN `p_ubicacion` VARCHAR(100), IN `p_capacidad` INT)   BEGIN
    INSERT INTO estadios (nombre, ubicacion, capacidad)
    VALUES (p_nombre, p_ubicacion, p_capacidad);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_gol` (IN `jugador_id` INT, IN `partido_id` INT, IN `tipo_gol` VARCHAR(50))   BEGIN
  INSERT INTO goles (id_jugador, id_partido, tipo)
  VALUES (jugador_id, partido_id, tipo_gol);
END$$
...
INSERT INTO `estadios` (`id_estadio`, `nombre`, `ubicacion`, `capacidad`) VALUES
(1, 'Hernan ramirez villega', 'pereira', 36000),
(2, 'Pascual guerrero', 'Cali', 39000);
...
INSERT INTO `goles` (`id_gol`, `id_partido`, `id_jugador`, `tipo`) VALUES
(1, 1, 2, 'Penal'),
(2, 2, 2, 'tiro libre');
...
--
-- Estructura Stand-in para la vista `vista_estadios`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_estadios` (
`id_estadio` int(11)
,`nombre` varchar(100)
...
--
-- Estructura para la vista `vista_estadios`
--
DROP TABLE IF EXISTS `vista_estadios`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_estadios`  AS SELECT `estadios`.`id_estadio` AS `id_estadio`, `estadios`.`nombre` AS `nombre`, `estadios`.`ubicacion` AS `ubicacion`, `estadios`.`capacidad` AS `capacidad` FROM `estadios` ;
